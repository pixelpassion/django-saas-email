...
class AbstractMailTemplate(models.Model):
    ...
    text_template = models.TextField(
        ...
        help_text=_(
            "This is an optional field for adding a custom text-only "
            "version of this template. If left blank, the plaintext email "
            "will be generated dynamically from the HTML when needed."
        ),
        default="",
        null=True,
        blank=True,
    ...
    def render_subject(self, context):
        """
        Take a list of values (inputs) and format the subject template,
    ...
    def render_with_context(self, context):
        ...
        If the text_template field is empty, it will dynamically generate the text
        version from the HTML output.
        """
    ...
    def html_to_text(self, html_string):
        """
        Convert a string containing HTML into a string with plaintext only.

        This is currently done with html2text, which converts HTML into valid Markdown.
        If no text_template exists, make_output() will use this method to generate the
        text-only output.  This method should not be called externally.
        """
...
class AbstractMail(models.Model):
    ...
    def send(self, sendgrid_api=False):
        ...
        It can be called directly, but is usually called asynchronously with
        tasks.send_asynchronous_mail.
